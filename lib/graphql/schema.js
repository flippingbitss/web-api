  ...
  Mutation: {
    ...
    submitPost(root, { title, body }, context) {
      ...
      return Promise.resolve()
        .then(() =>
          context.Posts.getByTitle(title).catch(() => {
            console.log(
              `No match found for post "${title.substring(0, 20)}" -> INSERTING`
            );
          })
        )
        .then(() => context.Posts.submitPost(title, body, context.user.userid))
        .then(post => context.Posts.getById(post));
    },
    ...
    submitComment(root, { postId, content }, context) {
      ...
      return Promise.resolve().then(() =>
        context.Comments.submitComment(postId, content, context.user.userid)
      );
    ...
    vote(root, { postId, type }, context) {
      ...
      return context.Posts
        .submitVote(postId, voteValue, context.user.userid)
