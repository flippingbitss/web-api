...
const rootResolvers = {
  Query: {
    ...
    allAnswersFromUser(root, { postedById, offset, limit }, context) {
      // Ensure API consumer can only fetch 20 items at most
      const protectedLimit = limit < 1 || limit > 20 ? 20 : limit;
      return context.Answers.getAllAnswersByUserId(
        postedById,
        offset,
        protectedLimit
      );
    },
    events(root, _, context) {
      // Ensure API consumer can only fetch 20 items at most
      return context.Events.getEventsByUserId(context.user.id);
    },
  ...
  Mutation: {
      ...
      return Promise.resolve()
        ...
        .catch(e => {
          return context.Posts
            .uploadImage(image)
            .then(imageUri =>
              context.Posts.submitPost(title, body, imageUri, context.user.id)
            )
            .then(([{ id }]) => {
              return context.Users
                .updateReputation(context.user.id)
    ...
    addEvent(root, { name, selectedDate }, context) {
      if (!context.user.id) {
        throw new Error("Must be logged in to create event");
      }
      let date = new Date(Date.parse(selectedDate))
      return context.Events.addEvent(name, date, context.user.id);
    },
    removeEvent(root, { id }, context) {
      if (!context.user.id) {
        throw new Error("Must be logged in to create event");
      }
      return context.Events.removeEvent(id, context.user.id);
    },
