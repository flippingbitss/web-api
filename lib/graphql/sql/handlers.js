...
function formatRows(query) {
  return query.then(rows => {
    if (!rows) throw new Error("No items found");
...
export class Events{
  getEventsByUserId(id){
    let query = knex(EVENT_TABLE).where({postedBy: id})
    return query.then((rows) => rows)
  }

  addEvent(name,selectedDate,userId){
    return knex(EVENT_TABLE).insert({name,selectedDate,postedBy: userId}).then(() => this.getEventsByUserId(userId))
  }

  removeEvent(id,userId){
    return knex(EVENT_TABLE).where({id}).delete().then(() => this.getEventsByUserId(userId))
  }
}
...
export class Posts {
  ...
  search(query) {
    ...
    if (query.trim().length < 3) 
      throw new Error("query string invalid or length less than 3");

    let sqlQuery = knex.raw(
      `SELECT * FROM ${POST_TABLE} JOIN (
        SELECT id FROM(
          SELECT ${POST_TABLE}.id as id,
             to_tsvector(${POST_TABLE}.title) || 
             to_tsvector(${POST_TABLE}.body) as document
             FROM ${POST_TABLE} GROUP BY ${POST_TABLE}.id) p_search
             WHERE p_search.document @@ to_tsquery('${query}')) results on results.id = ${POST_TABLE}.id limit ${limit}`
    );
    return sqlQuery.then(({rows}) => {
      if (!rows) throw new Error("No items found");
      return rows 
    });
  }
  ...
  uploadImage(image) {
    console.log(image);
    if (!image) return new Promise((res, rej) => res(""));
